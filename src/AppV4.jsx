...
function SettingsPage({ settings, reloadSettings, toast }) {
  ...
  const [destinations,setDestinations] = useState(settings.destinations); const [actors,setActors]=useState([]); const [sync,setSync]=useState(null);
  useEffect(()=>setDestinations(settings.destinations),[settings.destinations]);
  ...
  async function addDestination(){const name=prompt('Nom de la destination / département :');if(!name)return;const p=window.createurAO?.chooseFolder?await window.createurAO.chooseFolder(''):'';if(!p)return;await api('/api/destinations',{method:'POST',body:JSON.stringify({name,path:p})});await reloadSettings();}
  async function removeDestination(id){if(!confirm('Supprimer cette destination des réglages ?'))return;await api(`/api/destinations/${id}`,{method:'DELETE'});await reloadSettings();}
  return <main className="content settings-page"><header className="page-title"><div><span className="eyebrow">Configuration</span><h1>Réglages</h1></div></header>
    <section className="settings-panel"><header className="panel-title"><div><span className="eyebrow">Multi-postes</span><h2>Base maître</h2></div><button className="secondary-button" onClick={syncNow}><Icon name="refresh" size={15}/>Synchroniser</button></header><p>Chaque poste garde son SQLite local rapide. Le dossier maître sur le serveur contient le journal partagé et synchronise les postes toutes les 5 secondes, sans serveur applicatif.</p>
      <div className="v4-settings-grid"><Field label="Dossier serveur maître" wide><div className="v4-path"><input value={shared.masterRoot} onChange={e=>setShared(v=>({...v,masterRoot:e.target.value}))} placeholder="Choisir le dossier réseau"/><button type="button" onClick={()=>browse('masterRoot')}><Icon name="folder"/></button></div></Field><Field label="Dossier Gagnés"><div className="v4-path"><input value={shared.wonPath} onChange={e=>setShared(v=>({...v,wonPath:e.target.value}))}/><button type="button" onClick={()=>browse('wonPath')}><Icon name="folder"/></button></div></Field><Field label="Dossier Perdus"><div className="v4-path"><input value={shared.lostPath} onChange={e=>setShared(v=>({...v,lostPath:e.target.value}))}/><button type="button" onClick={()=>browse('lostPath')}><Icon name="folder"/></button></div></Field></div>
      <div className="v4-settings-footer"><span>PC : <strong>{settings.peerId}</strong>{sync?.lastSync ? ` · dernière synchro ${new Date(sync.lastSync).toLocaleTimeString('fr-FR')}` : ''}</span><button className="primary-button" onClick={saveShared}>Enregistrer</button></div>
    </section>
    <section className="settings-panel"><header className="panel-title"><div><span className="eyebrow">Organisation</span><h2>Destinations / départements</h2></div><button className="secondary-button" onClick={addDestination}><Icon name="plus" size={15}/>Ajouter</button></header><div className="v4-dest-list">{destinations.map(d=><div key={d.id}><strong>{d.name}</strong><code>{d.path}</code><button onClick={()=>removeDestination(d.id)}><Icon name="trash" size={14}/></button></div>)}</div></section>
    <section className="settings-panel"><header className="panel-title"><div><span className="eyebrow">Identification</span><h2>Personnes</h2></div><Icon name="users"/></header><p>Le PC identifie automatiquement l’auteur. Le nom affiché est modifiable ici. Cela ne limite jamais les départements auxquels une personne peut écrire.</p><div className="v4-actor-list">{actors.map((a,i)=><div key={a.pcId}><code>{a.pcId}</code><input value={a.displayName} onChange={e=>setActors(v=>v.map((x,j)=>j===i?{...x,displayName:e.target.value}:x))} placeholder="Nom de la personne"/><button className="secondary-button" onClick={()=>saveActor(a)}>Enregistrer</button></div>)}</div></section>
  </main>;
}
...
export default function AppV4() {
  ...
  return <div className="app-shell"><Sidebar active={active} setActive={setActive}/>{page}<Toast key={toastValue?.key} value={toastValue} close={()=>setToastValue(null)}/></div>;
}
