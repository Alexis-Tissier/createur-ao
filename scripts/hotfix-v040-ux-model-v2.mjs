...
function replaceToEnd(text, start, replacement, label) {
  const i = text.indexOf(start);
  if (i < 0) throw new Error(`Start marker missing: ${label}`);
...
`, 'parse optional commercial/quote');
...
`, 'SettingsPage');
  ...
  s = replaceToEnd(s, 'export default function AppV4()', `export default function AppV4() {
...
`, 'automatic due date backfill');

  s = replaceBetween(s, 'function publicSettings() {', 'function actorName(', `function publicSettings() {
  return {
...
`, 'snapshot due date');
  ...
  s = replaceBetween(s, 'async function scanStatusDirectory(', 'let scanBusy = false;', `async function findNumberedChild(root, number) {
...
`, 'status scanner helpers');

  s = replaceBetween(s, 'let scanBusy = false;', 'const app = express();', `let scanBusy = false;
...
`, 'status scanner');
  ...
  s = replaceBetween(s, "app.put('/api/settings/shared'", "app.get('/api/sync/status'", `app.put('/api/settings/shared', async (req, res) => {
...
`, 'master settings endpoint');
  ...
  s = replaceBetween(s, "app.post('/api/offers'", "app.patch('/api/offers/:uid'", `app.post('/api/offers', async (req, res) => {
  let finalPath = null;
  try {
...
`, 'create offer endpoint');
  ...
  s = replaceBetween(s, "app.patch('/api/offers/:uid'", "app.delete('/api/offers/:uid'", `app.patch('/api/offers/:uid', (req, res) => {
  try {
...
`, 'inspect defaults');
  ...
  s = replaceBetween(s, "app.post('/api/transfer/execute'", "app.post('/api/bootstrap/import'", `app.post('/api/transfer/execute', async (req, res) => {
...
`, 'transfer endpoint');
  ...
  s = replaceBetween(s, "app.post('/api/bootstrap/import'", "app.put('/api/tree'", `app.post('/api/bootstrap/import', (req, res) => {
  try {
...
// -------------------- Compact CSS --------------------
{
  const file = 'src/styles-v4.css';
  let s = fs.readFileSync(file, 'utf8');
  if (!s.includes('/* compact-v4-2 */')) {
...
// -------------------- Tests --------------------
{
  const file = 'tests/folders-v4.test.mjs';
  let s = fs.readFileSync(file, 'utf8');
...
{
  const file = 'tests/workflow-v4.test.mjs';
...
test('échéance=date AO, transfert utilise destination dédiée et dossiers 2/3/4/5', {timeout:25000}, async()=>{
...
console.log('v0.4 compact/destination workflow hotfix applied');
