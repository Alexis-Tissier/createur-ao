...
if old in app:
    ...
    raise SystemExit('Logo attendu introuvable')
app_path.write_text(app, encoding='utf-8')

workflow_rel = Path('.github/workflows/release-v0.4.1-beta.9.yml')
workflow_path = ROOT / workflow_rel
workflow = workflow_path.read_text(encoding='utf-8')
marker = "      - 'release-beta9-inline.trigger'\n"
if marker not in workflow:
    workflow = workflow.replace("      - 'package.json'\n", "      - 'package.json'\n" + marker, 1)
    workflow_path.write_text(workflow, encoding='utf-8')
subprocess.run(['git', 'add', str(workflow_rel)], cwd=ROOT, check=True)

print('beta9: logo inline prêt et déclencheur release ajouté')
